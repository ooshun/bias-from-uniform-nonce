...
SignatureLeak sign_msbs_leak(Domain pp, mpz_class d, int M, uint32_t leak, gmp_randclass& rand, mpz_class msbs);
SignatureLeak sign_msbs_leak_error(Domain pp, mpz_class d, int M, uint32_t leak, gmp_randclass& rand, const std::vector<int> &error_vector);
std::vector<int> sign_error_vector(const uint32_t &l, const std::vector<int> & msbs_vector, std::uniform_real_distribution<> &dis, std::mt19937 &gen, const float &error);
std::vector<int> calc_msb_vector(const mpz_class &msbs, const int &leak);
